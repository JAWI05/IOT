...
#define TOPICO_PUBLISH "AmongUs2Confirmado"    /*tópico MQTT de envio de informações para Broker
...
#define ID_MQTT "Among_us1"     /* id mqtt (para identificação de sessão)
...
/* Constantes */
const char* SSID = "ESEG-WIFI"; // coloque aqui o SSID / nome da rede WI-FI que deseja se conectar
...
void envia_medicoes_para_serial(float UmidSolo, float temp_lida, float umid_lida);
void init_wifi(void);
void init_MQTT(void);
void reconnect_wifi(void); 
void reconnect_MQTT(void);
void verifica_conexoes_wifi_e_MQTT(void);
void envia_informacoes_por_mqtt(float UmidSolo, float temp_lida, float umid_lida);
...
void envia_medicoes_para_serial(float UmidSolo, float temp_lida, float umid_lida) 
{
  /*Converte o valor lido em porcentagem de 0 a 100*/
  UmidSolo = map(UmidSolo, 1023, 0, 0, 100);
  
  char mensagem[200];
  char i;

  /* pula 50 linhas, de forma que no monitor serial seja exibida somente as mensagens atuais (impressao de refresh de tela) */
  for(i=0; i<50; i++){
      Serial.println(" ");
  }
  ...
  /* - umidade do solo */
  memset(mensagem,0,sizeof(mensagem));
  sprintf(mensagem,"Umidade do Solo: %.2f %", UmidSolo);
  Serial.println(mensagem);
}
...
void envia_informacoes_por_mqtt(float UmidSolo, float temp_lida, float umid_lida)
{
  ...
  //constrói mensagens e as envia
  sprintf(mensagem_MQTT,"Temperatura: %.2f C, Umidade atual: %.2f/100, Temperatura maxima: %.2f C, Temperatura minima: %.2f C, Umidade do Solo: %.2f %", temp_lida, 
                                                                                                                              umid_lida,
                                                                                                                              temperatura_max,
                                                                                                                              temperatura_min,
                                                                                                                              UmidSolo);
...
void loop() {
  ...
  umidsolo_lida = analogRead(UMIDPIN);
...
/* se houve falha na leitura do sensor, escreve mensagem de erro na serial */
  ...
  if ( isnan(umidsolo_lida) ) {
    Serial.println("Erro ao ler sensor de umidade do solo");
  }
  else
  {
    ...
    envia_medicoes_para_serial(umidsolo_lida, temperatura_lida, umidade_lida);
    envia_informacoes_por_mqtt(umidsolo_lida, temperatura_lida, umidade_lida);
  }
